...
test("SmartImg retries image candidates once with a cache-busting query before surfacing an error", () => {
  ...
  assert.match(smartImg, /srcSet=\{retryJpegSrcSet\}/);
});

test("homepage keeps only the hero and first featured card on the high-priority path", () => {
  assert.match(homePage, /<HeroStrip cases=\{stripCases\} limit=\{8\} \/>/);
  assert.match(homePage, /priorityCount=\{1\}/);
  assert.doesNotMatch(homePage, /priorityCount=\{4\}/);
  assert.doesNotMatch(homePage, /priority:\s*true/);
});
