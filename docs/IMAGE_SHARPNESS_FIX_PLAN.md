...
### §11.5 追加：模板墙改自然比例瀑布流（2026-09-12 深夜，用户二次反馈「竖图被裁、字挡图」）

**§11.4 的遗留错误**：卡片改图片主导后仍用**统一比例框**（桌面 16:10 / 手机 4:3）+ object-cover——48 张封面里 22 张竖图被裁掉一半以上，"所有图比例都一样" 是裁切框强制的假象；详情页 hero 同样 16:10 裁切。

**根治（`e5d71a25`）**：封面**按原始比例直出**（preserveAspectRatio，不裁切）。
- 新建 `TemplateGrid`：行优先 1px 网格测量瀑布流（与 /cases、/lab 同技术，防追加乱序），套 `.masonry-dense` 阶梯（2/3/4/5/6/7 列，卡 ≤260px 保小封面填充率）。
- CSS 密集阶梯由 `.masonry-lab` 更名 **`.masonry-dense`**（lab+模板两个无覆盖层墙共用，语义中性；守卫不变：`.masonry-feed` 永不携规则）。
- 卡片文字降到最少：封面 + 小标题（12.5px 两行）+ 复制模板；分类/描述/变量/适用方向/展开只在详情页。
- 详情页 hero 取消固定比例：`max-h-[76vh]` 完整展示竖图（387×516 竖图实测原比例全显）。
- rig 断言升级：**封面宽高比必须多样**（8 张 ≥4 种比例，手机 ≥3 种）——防止统一裁切框回归；`_hands_on.js` 的 lab 详情探针改为「最大 raster ≥1500px」（旧选择器在新布局下匹配到 40px 图标）。

**验证**：357/357；线上（`BASE=https://taostudioai.com`）rig 全绿 + hands-on 全绿；实测桌面 7 列比例 1.5/1/0.6/0.67/1.78/0.75/0.56/0.67，手机 2 列同样各异，竖图详情页 387×516 完整显示。

### §11.6 追加：三墙统一密集两列（2026-09-12 深夜，用户三次反馈「手机间距大/案例为什么单列」）

**用户质问的两点**：①手机端模板/4K 间距仍大 ②案例单列而模板/4K 双列，不一致。实测墙内间距本就是 12px，感知差距来自卡片内部文字块（标题+按钮+内边距 ~80px）与**案例墙的 1 列全屏卡**。

**根治（`8211fc9a`）**：
- 案例卡手机端改「封面 + 一行标题 + 复制按钮」极简 footer（**文字在图下方**），预览/更多收进详情页与长按菜单——v1 回归（§11.1）的结构性原因（文字钉图需图高 ≥165px）不复存在，案例墙得以并入 `.masonry-dense`。
- 三墙（案例/模板/4K）统一：手机 2 列、桌面同阶梯（7 列@1920）、手机间距 **12→8px**（双类命中 ready 网格）、卡片内边距收紧、4K 卡去掉日期·尺寸行、模板标题一行。
- 案例卡 `sizes` 手机 100vw→50vw（507px 需求 → 640 档 → ~125% 填充）。
- 守卫同步：案例墙必须 dense、**手机端禁止文字钉图**（`absolute bottom-0 sm:hidden` 模式出现即测试红）、8px 双类命中、dense 源序。

**验证**：357/357；线上（`BASE=https://taostudioai.com`）三墙手机端实测 2 列 + 间距 8-9px + 零水合错误；rig 与 hands-on 全绿。
