...
## 九、关于腾讯云 COS：暂不启用（2026-05-19 决策）

### 背景

5/18 开通了腾讯云 COS HK（bucket `gpt-image-2-1259488227`，region `ap-hongkong`），原意是给国内移动端图片再加一层就近 CDN。当时同步落地了：

- `scripts/upload-cos.mjs`：`public/uploads/` → COS 的幂等增量上传脚本
- `.env.example` / `.env.local` 里的 `VITE_COS_*` + `COS_*` 配置占位
- `vercel.json` 里给 `/uploads/(.*)` 配了一年缓存（这条与 COS 无关，独立有用）

### 现状盘点

| 项 | 实际接入度 |
|---|---|
| `src/lib/img.ts` 运行时是否读 `VITE_COS_*` | **没有**。只认 `/images/*`、`/assets/*`、`/uploads/*` 三类同源路径 |
| `SmartImg.tsx` / `ImageLightbox.tsx` 是否走 COS | **没有**。本地路径直接 `return src`，零运行时改写 |
| CI（`deploy.yml` / `sync.yml`）是否调用 `upload-cos.mjs` | **没有**。只能 `npm run upload-cos` 手动执行 |
| COS bucket 当前内容 | **空**（脚本从未运行过） |

也就是说：开通了，但代码层没有任何一处真正用到。

### 决策：保留 bucket、停用密钥、不动代码

理由（按重要性）：

1. **`build-images.mjs` 已经把 COS 想解决的问题解决了**。每张图在 prebuild 阶段被 sharp 重编为 1200px / JPEG q=80 / mozjpeg / progressive，写到 `public/images/case<id>.jpg`，并就地改写 `cases.json` 的 `imageUrl`。产物：470 张、76.4 MB、平均 166 KB。
2. **同源 + Vercel edge HIT 已经是最优解**。第七节实测 `X-Vercel-Cache: HIT` + `hnd1`，国内 RTT 50–80ms。引入 COS 等于：
   - 多一次 DNS + TCP + TLS 握手（首图首屏多 ~150–300ms）
   - 失去 Vercel edge 缓存（COS HK 是单 region，海外用户反而更慢）
   - HTTP/2 同源复用断开
3. **COS 想要真正发力得套腾讯云 CDN**（COS 原域名国内速度不稳），那是另一笔接入工作和持续费用。
4. **改造成本高于潜在收益**。要让 COS 出活，至少要改 `lib/img.ts` + `SmartImg.tsx` + `ImageLightbox.tsx` + CI 加 `upload-cos` step + 接入 `imageMogr2/format/webp/thumbnail` 才能拿到带宽优势。这些改动博的是一个**未经实测**的提速。

### 已执行动作

- [x] `.env.local` 清空 `COS_BUCKET / COS_REGION / COS_SECRET_ID / COS_SECRET_KEY`，并在文件头加注释说明决策出处
- [x] `.env.local` 同步清空 `VITE_COS_BUCKET / VITE_COS_REGION / VITE_COS_HOST`（前端没有消费者，留着只会误导）
- [x] `scripts/upload-cos.mjs`、`vercel.json`、`workflows/*` **保持不变**——脚本本身是幂等的，作为"未来可能用得上的工具"留在仓库

### 用户侧手工确认事项（不能自动化的部分）

- [ ] **腾讯云 CAM 控制台作废之前在对话里出现过的 SecretId**（已通过 GitHub Push Protection 拦截、本文档不再保留原值）。按 `upload-cos.mjs` 注释惯例（"previous key was burned"）需立即停用。
- [ ] CAM 里子账号本身可禁用，bucket 留着不收存储费（76 MB 一年几分钱）。

### 何时回头考虑接入 COS

只有同时满足：

- 国内多地（电信/联通/移动 + 一线 + 二三线城市）实测 Vercel `/images/*` TTFB 中位数 > 800ms；并且
- 愿意承担 `imageMogr2` 格式协商和 srcSet 重构的复杂度；并且
- 同步接入腾讯云 CDN（COS 原域名直连不够稳）

在那之前，**Vercel + 烘焙好的本地 JPEG** 就是当前规模下的标准答案。
...
## 十、移动端图片管线深度优化（2026-05-19 第二轮）

### 背景

第八节迁到 Vercel hnd1 后，CDN 链路已是最优（同源 + edge HIT）。但用户反馈
"中国区移动端体感还是慢"。第二轮诊断（详见 `git log` 上下文）发现真正的
瓶颈不在网络层，而在两个之前没暴露的产品层问题：

1. **`SmartImg.tsx` 对本地 /images/* 强制禁用 srcset**。代码注释 "no real
   width variants on disk" 是 2026-08 重写时的事实判断——当时管线只生成一
   档 1200 px JPEG。结果：手机 50vw 卡片（DPR 3 → 物理 480 px）也下载 1200
   px 全图。
2. **Google Fonts (`fonts.googleapis.com`) 在国内手机经常阻塞 5-15 秒**。
   `display=swap` 让正文先渲染，但 `<link rel="preload" as="style">` 仍占用
   连接配额，且当 woff2 实际下载失败时 Instrument Serif 头条永远走兜底字体
   ——视觉上文字会"换字体跳一下"。

对标站 `gpt-image2.canghe.ai` 的实测数据反过来佐证了诊断方向：
- 它的图也是 Vercel sin1 同源 JPEG，**不是中国区 CDN**
- 它的图甚至比我们大（前 12 张 3.3 MB vs 我们 2.0 MB）
- 但它首页 HTML 仅 564 字节（纯 SPA 空壳），首屏 0 个 `<img>`，先把页面壳
  渲出来再异步加载

我们是 SSG 461 页，HTML 153 KB——SEO 优势保留，不能改成 SPA。所以方向是
**让管线本身把图变小、首屏字体不卡 Google**，而不是动产品形态。

### 方案与实施

| 改动 | 文件 | 收益 |
|---|---|---|
| sharp 多档 WebP（320/480/640/960） | `scripts/build-images.mjs` | 467 张图 × 4 档 = 1880 个 WebP |
| 本地图变体助手 | `src/lib/img.ts` 新增 `localWebpSrcSet` / `pickLocalWebp` / `localJpegSrcSet` / `LOCAL_WEBP_WIDTHS` | 给 srcset 提供数据源 |
| `<picture>` 走真 srcset | `src/components/SmartImg.tsx` | 浏览器按 `sizes` 自动选档 |
| 自托管 Instrument Serif | `public/fonts/instrument-serif-{regular,italic}.woff2` 各 ~15 KB + `index.html` `@font-face` + preload | 切断 Google 依赖 |
| 删 `cases.json` preload | `index.html` | 省一次重复 200 KB 请求（数据已 import 进 bundle） |
| 删 Google Fonts preconnect/link | `index.html` | 省两个境外握手 |
| woff2 长缓存 + CORS | `vercel.json` `/fonts/(.*)` | edge immutable 1 年 |

`localWebpSrcSet` 关键设计：调用方传 CSS 宽度梯度（如 `[280, 380, 540]`），
内部 **snap-up 到最近的 on-disk 档位**——即 280 → 320, 380 → 480, 540 → 640。
不要求调用方知道管线档位，避免历史 ladder 默写错变成"全部 fallback 到 1200"。

### 验收数据

```
本地 build 通过：tsc -b ✓ + vite-react-ssg build ✓
产物：dist/ 261 案例页 + 19 分类页 + 7 静态页 + sitemap = 完整保留
首页 HTML：153 KB（无变化，视觉与原版一致）
首页 <picture> 标签：25 个（hero + featured + templates 全覆盖）
首页 srcset 属性：29 个
首页 unique WebP refs：21 个

每档 WebP 平均体积：
  320w: 25 KB（首页 56×56 缩略 / DPR 1 卡片）
  480w: 49 KB（DPR 3 移动端 50vw 卡片）
  640w: 75 KB（桌面 33vw 卡片）
  960w: 117 KB（详情页大图）
  1200 JPEG: 167 KB（兜底 / OG 卡片 / no-WebP 浏览器）

实际首屏改进（移动端 hero 4 张瀑布图）：
  Before: 4 × 167 KB JPEG = 597 KB
  After:  4 × 49 KB WebP-480 = 138 KB
  Saved:  77%
```
...
```
JPEG 474 张 77 MB（保留作 fallback / OG / no-WebP）
WebP 1896 张 123 MB（覆盖 4 档 × 474 源）
共 200 MB，Vercel 静态资源限额（每文件 50 MB）远未触达。
```
...
- 不让 `/images/*` 走 wsrv（北美 POP，慢）
- 不接 COS HK + 腾讯 CDN（多一跳 + 失去 Vercel edge HIT，参见 §九）
- 不上 ICP 备案（DAU < 1000 不做，参见 §四）
- 不上 AVIF（sharp encode 慢 4-6×，省 ~10% 不值）
- 不改产品形态变 SPA（保 SEO 461 页静态化是核心竞争力之一）

### 后续观察项

- [ ] 国内手机 4G 实测体感（带数字：LCP / FCP / 首屏可交互时间）
- [ ] Vercel Analytics 看 Real Experience Score
- [ ] 跑一遍 PageSpeed Insights 拿 Lighthouse 分（应能从 ~70 上到 90+）
- [ ] 如果实测仍不达预期：考虑 Phase 4（拆 cases.json 瘦版到 cases-index.json）

### 一句话总结

**这次 P0 不靠换基础设施、不靠减内容**，靠 build pipeline 把同样多的图
变成同样视觉但带宽小 60-77% 的 WebP 多档。视觉零变化，体感大改善。
