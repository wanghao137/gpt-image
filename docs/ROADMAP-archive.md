# GPT-Image 2 中文案例库 · 历史交接路线图（已归档）

> ⚠️ **这份文档是 2026-05-17 的快照**，当时项目处于"商业化闭环 MVP"阶段。
> 后续（2026-05-19 之后）产品方向回到了**纯案例画廊（gallery-first）**：
> `/services`、`/guide`、`/agents`、`WeChatCTA` 等商业化页面/组件已删，
> 首页恢复成 hero + 分类磁贴 + 精选 + 模板 teaser 的形态。
>
> 当前架构现状请看 [`ARCHITECTURE.md`](./ARCHITECTURE.md)。
> 部署 / 性能改造的连续日志请看 [`PERF_PLAN.md`](./PERF_PLAN.md)。
>
> 本文档保留作为**工程历史**，里面的接手手册、SEO 实现、技术债清单仍有
> 参考价值；但**任何"立刻要做"的 P0 任务都已不再适用**——切勿照单执行。
